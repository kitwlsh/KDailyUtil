...
🟢 **vc11 첫 코드가 09-21에 들어갔다**(장문 갈라 보기 + 출처 표기). **다음은 실기기 확인이다.**
...
| 다음 행동 | ① **F-9 실기기 확인**(오늘 것) → ② **밀린 확인 5건**(§부록 G) → ③ vc11 나머지(EWPM) |

### 🔴 이 세션이 할 일 세 가지

1. 🆕 **F-9 실기기 확인** — 길이 필터 칩 · 출처 문구 · **장문 훈련**(📜 1,100자가 와 있어 **F-7을 겸한다**)
   → [doc/NEXT_SESSION.md §부록 F-9](doc/NEXT_SESSION.md) · 디버그 APK는 빌드해 뒀다
2. **밀린 확인 5건** — `aiModel` 레버 · 데이터 보안 칸 · edge-to-edge · 지문이 날짜마다 바뀌는지 ·
   깨끗한 계정 신규 설치. 🔴 **순서를 한 자리로 모았다** → [§부록 G](doc/NEXT_SESSION.md)
   (되돌릴 것 먼저 · **앱 지우는 일은 맨 마지막**)
3. 🟡 **vc11 나머지** — 유효 속도(EWPM) + «읽은 지문이 영영 재독» 문제

🔴 **vc11도 «실기기 확인 → 버전 올리기 → 빌드 → 서명 확인 → 업로드» 순서를 지킨다** —
