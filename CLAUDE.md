...
## 🎬 지금 할 일 (**2026-09-17 갱신**)

🔴 **신규 세션은 [doc/NEXT_SESSION.md](doc/NEXT_SESSION.md)의 머리말 세 줄부터 읽는다.** 아래는 요약이다.

### 한 줄 요약

🎉 **v1.7.2(vc10)가 09-17에 라이브가 됐다. 기다릴 심사가 0건이다 — 이제 «밀린 확인»과 vc11이 남았다.**

| | |
|---|---|
| 스토어 | 🎉 **v1.7.2 (vc10)** 라이브(**2026-09-17** · 09-16 업로드 → 다음날) |
| 저장소 | ✅ `main` = `origin/main` (KDailyUtil · korean_quiz_data 둘 다 · 09-17 실측) |
| 소스 버전 | ✅ **`versionCode 10` / `1.7.2`** = 스토어와 같다. 🔴 **vc10은 소모됐다 — 다음 업로드는 vc11** |
| 단위 테스트 | **117건 통과 · 실패 0**(09-17 실측) · ✅ 실기기 확인 **F-1~F-8 통과**(F-7만 09-19 대기) |
| 다음 행동 | ① **밀린 확인 5건** → ② **09-19(토) 첫 장문 지문** → ③ **vc11 착수(EWPM)** |

### 🔴 이 세션이 할 일 세 가지

1. **밀린 확인 5건** — `aiModel` 레버 · 데이터 보안 칸 · 깨끗한 계정 신규 설치 · edge-to-edge 눈확인 ·
   📖 **지문이 날짜마다 바뀌는지**(지문이 **10편**이 됐으므로 이제야 확인이 된다)
2. 📜 **09-19(토) 첫 장문 지문** — 그날에만 볼 수 있다 → [doc/NEXT_SESSION.md §부록 F-7](doc/NEXT_SESSION.md)
3. 🟡 **vc11 착수** — 유효 속도(EWPM) + «읽은 지문이 영영 재독» 문제

🟢 **코드를 건드려도 되는 상태다**(스토어=저장소=소스). 🔴 **다만 vc11도 «실기기 확인 → 업로드» 순서를 지킨다** —
09-14에 실기기에서만 잡힌 결함이 셋이었다(버튼 글자 세로 접힘 · 이름 없는 ✕가 영구 삭제 · 첫 설치에 빈 자리).
**단위 테스트는 이런 것을 못 잡는다.**
...
## 📌 이 프로젝트의 사실 (**2026-09-17 재검증**)

| 항목 | 값 |
|---|---|
| 스토어 게시본 | 🎉 **v1.7.2 (vc10)** · **2026-09-17 라이브** (v1.7.1(vc9)=09-09 · v1.7.0(vc8)=09-08) |
| 저장소 | ✅ **KDailyUtil · korean_quiz_data 둘 다 `main` = `origin/main`** (09-17 실측 · 로봇 커밋 1개를 `pull`로 받았다) |
| 소스 버전 | ✅ **`versionCode = 10` / `1.7.2`** = **스토어 게시본과 같다** · 🔴 **다음 업로드는 vc11** · 🟢 코드를 건드려도 된다 |
| 단위 테스트 | **117건 통과 · 실패 0**(09-17 재실행) — AiErrorMessage 15 · BriefingScheduler 7 · **DailyRecord 56** · GeminiFallback 11 · ReadingTrainingModule 11 · PassageLength 11 · PassageKey 5 · Example 1 |
...
| 퀴즈·지문 로봇 | ✅ **새 스케줄 2일 연속 성공**(09-17 07:19 KST) — 퀴즈 **565문항**(+5) · 지문 **10편**(+1) · `quotaWaits: 0`. ⏰ 예약 **04:40 KST** → 실제 **07:26**(09-16) · **07:19**(09-17) = GitHub 지연 **2시간 40분대로 안정**. ⚠️ **09-15분 지문은 빠졌다**(스케줄 교체일 · 예고된 것). 🔴 **토요일은 장문**(첫 장문 = **09-19**) |
