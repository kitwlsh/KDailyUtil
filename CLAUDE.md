...
🟢 **같은 날 2차로 더 들어갔다** — **허브 순서**(🏃 훈련 6종을 지문 카드 바로 아래로 · 「워밍업 보려면
지문을 다 스크롤」 해소) · **지문 길이 표시**(「224자 · 약 12초」) · **「치우기」→「숨기기」 개명**
→ [§할 일 0-E](doc/NEXT_SESSION.md). 테스트 **101건 통과**.
🔬 **실측으로 나온 숫자**: 시스템 지문 평균 **224자 = 300 WPM에서 약 11초**. **한 세션이 너무 짧다** —
주 1회 장문 지문이 다음 후보다(§할 일 0-E의 ⑤).

✅ **429 실측이 끝났다(2026-09-14 11:31 KST)** — 🔴 **한도는 «모델별»이었다.**
교대 2회차에서 `gemini-3.5-flash`=429(limit 5)인데 같은 순간 `gemini-3.6-flash`=200이다.
→ **08-25의 「한도는 키 단위」 전제가 뒤집혔고, 앱도 429에 폴백한다.** 회귀 테스트 3건을 정반대로 뒤집었다.
⚠️ **앱은 «자고 재시도»를 하지 않는다** — 로봇과 다른 선택이다(앞에 사람이 있고 대기가 28~38초다).
근거·실측표 = [doc/AI_KEY_NOTES.md §3-2](doc/AI_KEY_NOTES.md).

🟢 **④⑤까지 끝났다(09-14 3차)** — **「📚 내 지문」 별도 화면**(검색·편집·바로 시작 · 허브에서는 한 줄) ·
**주 1회 장문 지문**(일요일 800~1,200자 ≈ 40~60초 · 앱은 글자 수로 판정해 **「📜 긴 지문」** 표시).
🔴 **바로잡은 것**: 「장문이면 WPM 그래프가 왜곡된다」는 **틀렸다** — 기록되는 WPM은 측정값이 아니라
**슬라이더로 고른 설정 속도**다. 진짜 위험은 «완주 부담»(중간에 나가면 출석·연속이 빈다)이라
**시작 전에 「📜 긴 지문」이라고 알려 준다**.

🟢 **속도 기록의 신뢰성도 손봤다(5차)** — 🔴 **WPM은 «측정값»이 아니라 «슬라이더로 정한 설정값»**이었고
(읽었는지 앱은 모른다), 재독도 구분하지 않아 **암기한 지문으로 낸 숫자가 추천 목표까지 부풀리고** 있었다.
→ **재독은 속도 기록에서 뺀다**(출석·연속은 그대로) · 결과 화면이 그 이유를 말한다 · 통계에 «이 숫자의 뜻»을 적었다.
→ [§할 일 0-F](doc/NEXT_SESSION.md). 다음 판 후보 = **유효 속도(EWPM = 속도 × 이해도)**.

🔴 **버전은 여전히 안 올렸다**(사용자 지시) — `versionCode 9` / `1.7.1` 그대로다.

🔴 **남은 것 하나** — ① **실기기 확인**(검색·☆·「🔁 지난 지문」·되돌리기는 단위 테스트로 못 잡는다)
(검색·☆·「🔁 지난 지문」·「숨기기」·버튼 줄바꿈). 📦 디버그 APK가 `app/build/outputs/apk/debug/`에 있다.
