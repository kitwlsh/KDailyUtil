...
## 🎬 지금 할 일 (**2026-09-21 갱신**)

🔴 **신규 세션은 [doc/NEXT_SESSION.md](doc/NEXT_SESSION.md)의 머리말 세 줄부터 읽는다.** 아래는 요약이다.

### 한 줄 요약

🎉 **09-21에 밀린 확인이 0건이 됐고, vc11 1차분도 실기기를 통과했다. 이제 «올릴지»만 정하면 된다.**

| | |
|---|---|
| 스토어 | 🎉 **v1.7.2 (vc10)** 라이브(2026-09-17 · 09-16 업로드 → 다음날) |
| 저장소 | ✅ `main` = `origin/main` (KDailyUtil · korean_quiz_data 둘 다 · 09-21 실측) |
| 소스 버전 | 🔴 **소스 ≠ 스토어 게시본이 됐다** — `versionCode 10` / `1.7.2` 그대로지만 **vc11 몫 코드가 얹혔다**. 버전은 **실기기 확인 뒤에** 올린다 |
| 단위 테스트 | **124건 통과 · 실패 0**(09-21 · `PassageFilterTest` 7건 신설) · 🔬 뮤테이션 2건이 정확히 실패 |
| 실기기 | ✅ **09-21에 전부 통과 · 결함 0건** — vc11 몫(F-9) + **밀린 확인 5건**(08-12부터 이월되던 것) |
| 다음 행동 | 🔴 **«vc11을 지금 올릴까» 결정** → 올리면 **버전 → 빌드 → 서명 확인 → 업로드** |
...
1. 🟡 **「지문 신고」 한 줄 넣기**(vc11 후보 · 작다) — 🟢 **뉴스 브리핑·퀴즈에는 이미 있다**(09-23 코드 확인).
   지문·독서 훈련에만 없다 → 퀴즈 것을 거의 그대로 옮기면 된다 → [§할 일 0-J](doc/NEXT_SESSION.md)
2. 🔴 **«vc11을 올릴까» 결정 + 버전명** — 지금 올려도 막는 것이 없다(실기기 끝 · 밀린 확인 0건).
   올리면 **버전 올리기 → 빌드 → 서명 확인 → 업로드** 순서 그대로
3. 🟡 **EWPM은 다음 판(vc12)으로** — 키 없는 사용자 · 기록 형식 이전 · **표본이 작아 점수가 흔들리는 것**,
   걸림돌이 셋이라 따로 한 판이 맞다

💡 **권고**: vc11을 **«장문 필터 + 출처 표기 + 지문 신고»**로 묶으면 한 주제(**투명성·안전**)가 된다.

### 🆕 vc11에 들어간 것 (09-21~ · 진행 중)
...
**유효 속도(EWPM = 속도 × 이해도)** — 🔴 키 없는 사용자 문제 + 기록 형식 변경·데이터 이전이 얽혀 있다.
근거는 [doc/NEXT_SESSION.md §할 일 0-F](doc/NEXT_SESSION.md)에 정리해 뒀다.
...
| 소스 버전 | 🔴 **`versionCode = 10` / `1.7.2`인데 소스에는 vc11 몫이 얹혔다**(09-21~) — 스토어 게시본과 **내용이 다르다**. 올릴 때 **vc11로 올린다** |
| 단위 테스트 | **124건 통과 · 실패 0**(09-21) — AiErrorMessage 15 · BriefingScheduler 7 · **DailyRecord 56** · GeminiFallback 11 · ReadingTrainingModule 11 · PassageLength 11 · **PassageFilter 7** · PassageKey 5 · Example 1 |
...
```bash
./gradlew :app:testDebugUnitTest      # 단위 테스트 117건 (기기 불필요)
