...
## 🎬 지금 할 일 (**2026-09-14 갱신**)

🟢 **vc10 코드가 절반 들어갔다**(09-14 · 커밋 `11c6c43`) — 사용자 신고 3건을 코드로 원인 확인해 고쳤다:
**①** 기본 훈련을 사용자가 ☆로 정한다(「다음 훈련」 한 번에 몰래 바뀌던 것) · **②** 지문이 7일 뒤
«검색도 없는 창고»로 사라지던 것(검색 + 전체 목록 링크 + 7일에 하루 지난 지문) · **③** 「치우기」에
되돌릴 길이 없던 것. 여기에 **429 안내 문구**(§할 일 0-C 3번)가 얹혔다.
→ 근거·설계 판단 = [doc/NEXT_SESSION.md §할 일 0-D](doc/NEXT_SESSION.md). 테스트 **93건 통과 · 뮤테이션 4건 확인**.

🔴 **남은 것 둘** — ① **실기기 확인**(검색·☆·「🔁 지난 지문」·되돌리기는 단위 테스트로 못 잡는다)
② **429 실측**(진단 워크플로를 `701bde0`로 «교대» 방식으로 고쳐 올렸다 — **Actions에서 버튼만 누르면 된다**).
⚠️ **«매일 11:09에 429가 난다»는 더 이상 맞지 않다** — 로봇이 09-10·09-13 모두 `quotaWaits: 0`이다.

🔴 **버전은 안 올렸다** — `versionCode 9` / `1.7.1` 그대로다. 다음 업로드가 vc10이다.
...
🎉 **v1.7.1(vc9)이 2026-09-09에 출시됐다**(업로드 09-08 → 다음날 라이브 · 사용자 확인).
v1.7.0(vc8)도 09-08에 라이브가 됐다 — **이틀 연속으로 한 판씩 나갔다.**
🟢 **스토어와 저장소가 같다 — 밀린 출하가 0건이다.**
🔴 **`versionCode 9`는 소모됐다. 다음 업로드는 vc10이다**(반려되어 다시 올려도 10이다 — vc는 되돌릴 수 없다).

v1.7.1에 들어간 것 = «독서 훈련 시작까지 가는 길» 단축. 사용자 신고(「지문 고르고 훈련까지 스크롤이
너무 멀다 · 훈련 끝나면 또 그만큼」)에서 출발해 원인 3가지를 코드에서 확인해 고쳤다
→ [doc/NEXT_SESSION.md §할 일 0-B](doc/NEXT_SESSION.md). ✅ **실기기 확인은 업로드 전에 완료했다.**
⚠️ 스킴 규칙(`기능추가 → MINOR`)대로면 1.8.0인데 **사용자가 1.7.1로 결정했다** — 스킴이 바뀐 것은 아니다.

🟢 **이제 vc10 작업을 시작해도 된다** — 빌드해 둔 AAB를 올렸으므로 «소스와 산출물이 어긋나는» 제약이 풀렸다.

🔴 **신규 세션은 [doc/NEXT_SESSION.md](doc/NEXT_SESSION.md)부터 본다** — vc8 몫이던 확인이 아직 안 끝났다:
~~알람 이틀 연속~~(✅ **09-09 확인 완료**) · 지문이 매일 쌓이는지 · edge-to-edge 눈확인 ·
...
> 🔴 **올릴 때는 항상 `app/release/`의 파일을 쓴다** — `app/build/outputs/`는 다음 빌드에 갈아치워진다
...
2. 🎉 **v1.7.1 출시 완료(2026-09-09)** — 업로드 09-08 → **다음날 라이브**(사용자 확인)
   - 올린 파일 = [`app/release/kdailyutil-v1.7.1.aab`](app/release/kdailyutil-v1.7.1.aab) · 출시 노트 = `RELEASE_NOTES.md` §v1.7.1
   - ⏱ 심사 실측이 **3판 연속 «업로드 다음날»**이다(v1.6.1 · v1.7.0 · v1.7.1). 다음 판 일정도 이 값으로 잡으면 된다
3. 🔴 **출시가 끝나도 vc8 몫이던 확인은 그대로 남는다** — 부록 A
   - ✅ **가장 급했던 «알람 이틀 연속»은 09-09에 확인됐다** — 사용자 실기기에서 **09-08·09-09 연속 도착**.
     vc8의 핵심 수정이 실제로 도는 것이 증명됐다(예전 증상 = «첫날만 오고 끝») → **긴급 vc10은 필요 없다**
   - 나머지 = `aiModel` 레버 · 데이터 보안 칸 · 깨끗한 계정 신규 설치 · edge-to-edge 눈확인 · vc9 몫(부록 A §E)
4. 🟡 **vc10 후보 — AI 「한도 초과(429)」 대응** (09-08 발견 · **09-09에 실측·로봇은 해결**)
   - ✅ **로봇은 09-09에 되살렸다** — `QUIZ_MODEL` 레버(한도 5→20) + 429 재시도·모델 폴백.
     그날 14:48 실행이 `ok`(퀴즈 +5 · 지문 +1 · `quotaWaits: 0`)
   - 🔴 **앱은 아직 그대로다** — 사용자 키가 429를 맞으면 **폴백도 안 하고**, 안내 문구는
     «잠시 뒤 풀린다»고만 말한다. 실측으로 확정된 것 = **창은 수십 초 · 한도는 모델마다 다르다**
     (`gemini-3.8-flash`=5 vs `gemini-3.5-flash`=20)
   - 🟡 **폴백을 켜기 전에 «깨끗한 재시험» 1회가 남았다**(대조군이 오염됐다) → [doc/AI_KEY_NOTES.md §3-2](doc/AI_KEY_NOTES.md)
   ...
   - 🔴 **실측이 먼저다** — 「한도가 모델별인가 키 단위인가」를 확인해야 폴백을 켤지 정할 수 있고,
     그 실측은 **429가 실제로 나는 순간에** 해야 한다. 🕐 **퀴즈 로봇이 매일 «한국시간 11:09쯤» 그 상태가 된다**
     (예약 실행 = 02:09 UTC). 그 직후가 실측 창이다
   - 절차·근거 = [doc/AI_KEY_NOTES.md §3-2](doc/AI_KEY_NOTES.md) · 할 일 = [doc/NEXT_SESSION.md §할 일 0-C](doc/NEXT_SESSION.md)
   - 🟢 **이제 착수해도 된다** — 09-08에 코드를 안 건든 것은 소스가 빌드해 둔 AAB와 일치해야 했기 때문인데,
     그 AAB를 올렸으므로 제약이 풀렸다

---

## 📌 이 프로젝트의 사실 (2026-09-09 재검증)

| 항목 | 값 |
|---|---|
| 스토어 게시본 | 🎉 **v1.7.1 (vc9)** · **2026-09-09 라이브**(09-08 업로드 → 다음날 출시) |
| 저장소 | ✅ **스토어와 같다 — 밀린 분량 0건.** `main` = `origin/main` · 작업 트리 깨끗(09-09 실측) |
| 소스 버전 | `versionCode = 9` / `versionName = "1.7.1"` — **출시본과 일치**. 다음 업로드는 **vc10** |
| 단위 테스트 | **82건 통과 · 실패 0**(09-09 재실행 — 소스 무변경이라 Gradle이 `UP-TO-DATE`로 09-08 결과를 그대로 썼다) — AiErrorMessage 10 · BriefingScheduler 7 · DailyRecord 46 · GeminiFallback 9 · **ReadingTrainingModule 9** · Example 1 |
...
| 퀴즈·지문 로봇 | ✅ **되살렸다**(09-09 14:48 KST 실측 · `status: ok`) — 퀴즈 **530문항**(+5) · 지문 **3편**(+1) · `quotaWaits: 0`. 고친 방법 = **Actions 변수 `QUIZ_MODEL`=`gemini-3.5-flash`**(한도 5→20) + **429 재시도·모델 폴백**(`17dc9d5`) → [doc/AI_KEY_NOTES.md §3-2](doc/AI_KEY_NOTES.md) |
