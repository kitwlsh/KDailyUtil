...
| K장부 | `com.kitwlshcom.kjangbu` (확정, 소스 반영됨) | `ic_kjangbu.png` | AI 생활 기록·관리 장부(가계부·차계부·케어) | `com.kitwlshcom.kjangbu` | **라이브 v1.0.0(vc1, 2026-08-07 출시)** — 첫 출시본부터 §8 동적. 레지스트리 `comingSoon: true→false` 전환 완료 |
...
> 🎉 **전환 배포 완료 — 3개 앱 전부 라이브(2026-08-07).** **KDailyUtil vc6/v1.6 ✅**(08-04) · **KLotto645 vc13/v1.0.3 ✅**(v1.0.2부터 적용) · **K장부 vc1/v1.0.0 ✅**(08-07, 첫 출시본부터 동적이라 전환 배포가 애초에 없었다). **이제부터 신규 자매앱 추가에 어떤 앱도 재배포하지 않는다**(§8-9) — 설계 목표 달성.
...
| 앱 | 전환 상태 | 다음 배포 |
|---|---|---|
| **KDailyUtil** | ✅ **구현 + 🚀 배포 완료(2026-08-04 라이브)** — Compose + Coil. **실기기 검증 10/10 통과(§8-12)** | **vc6 / v1.6 (라이브)** |
| **KLotto645** | ✅ **구현 + 🚀 배포 완료(2026-08-03, v1.0.2부터 적용)** — XML/View. 이미지 라이브러리가 없어 `RemoteIconCache`(디스크 캐시 + `BitmapFactory`)를 직접 구현, **의존성 추가 없음**. 실기기/에뮬레이터 **T1~T10 전 항목 검증 통과** | **vc13 / v1.0.3 (라이브)** |
| **K장부** | ✅ **구현 + 🚀 출시 완료(2026-08-07 라이브)** — Compose + Coil. **첫 출시본(vc1)부터 동적** → 전환용 추가 배포가 애초에 없음 | **vc1 / v1.0.0 (라이브)** |

> 🎉 **전환 배포 완료 — 3개 앱 전부 라이브(2026-08-07).** 업데이트를 받은 사용자부터 원격 카드가 보인다. 아직 예전 버전(KDailyUtil v1.5 이하)에 머문 사용자에게는 업데이트가 깔릴 때까지 하드코딩 카드가 보이지만(K장부 카드 없음) 시간이 해결하는 문제다. **이후로는 영구히 JSON만으로 관리된다.**
...
> **Console 등록 현황(2026-08-03 교체 / 2026-08-04 갱신)**: 3개 앱 모두 위 GitHub Pages 주소로 교체 완료. **공개 스토어 페이지의 방침 링크는 새 버전이 게시될 때 함께 반영**되므로, 게시된 KDailyUtil(v1.6)·KLotto645(v1.0.3)는 이제 새 주소가 노출돼야 한다 — **스토어에서 실제로 새 주소인지 한 번 확인할 것**(아직 옛 Netlify 주소면 Console 값을 재확인). K장부는 첫 출시 검토 중이라 게시 시점에 반영된다.
