...
> ✅ **전환 배포도 사실상 완료(2026-08-04)** — 이 전환은 **각 앱이 한 번 배포돼야 효력이 생기는데**, **KDailyUtil vc6/v1.6 ✅ 라이브(2026-08-04)**, **KLotto645 vc12/v1.0.2부터 적용 → 현재 라이브 v1.0.3 ✅**, K장부는 첫 출시본(vc1)부터 동적이라 별도 전환 배포가 없다(⏳ 첫 출시 검토 중). 이후로는 **신규 자매앱 추가에 어떤 앱도 재배포하지 않는다**(§8-9).
...
| **K장부** | ✅ **구현 완료(2026-07-29)** — Compose + Coil. **첫 출시본(vc1)부터 동적** → 전환용 추가 배포가 애초에 없음 | ⏳ 첫 출시(vc1) 검토 중 |

> ✅ **전환 배포 완료 — 출시된 두 앱은 이미 동적으로 돈다(2026-08-04).** KDailyUtil v1.6·KLotto645 v1.0.2+가 모두 라이브이므로, **업데이트를 받은 사용자부터 원격 카드**가 보인다. 아직 예전 버전(KDailyUtil v1.5 이하)에 머문 사용자에게는 업데이트가 깔릴 때까지 하드코딩 카드가 보인다(K장부 카드 없음) — 이는 시간이 해결하는 문제다. 이후로는 영구히 JSON만으로 관리된다.
