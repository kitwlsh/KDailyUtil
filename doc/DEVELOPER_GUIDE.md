...
> ### 🚩 지금 상태 (신규 세션 필독)
> - 🕐 **v1.7.1 (versionCode 9) — 2026-09-08 업로드 완료·검토 중.** 🔴 vc9 소모됨 → 다음 업로드는 **vc10**.
>   들어간 것 = 독서 훈련 「시작까지 가는 길」 단축(지문 카드 시작 버튼 · 버튼 이름에 훈련명 ·
>   결과 화면 「다시·다음」 · 스크롤 보존 · 보관함 상한). ✅ 실기기 확인 완료.
>   ⚠️ 스킴대로면 1.8.0인데 **사용자가 1.7.1로 결정했다** — 스킴이 바뀐 것은 아니다.
> - **🎉 게시본 = v1.7.0 (versionCode 8) — 2026-09-08 출시·라이브**(09-07 업로드 → 다음날).
>   들어간 것 = 매일 새 지문 공급(기능) + 「새 문제 N개」 상한·복귀 사면 + 리텐션 한 판 + 503 대응.
>   🔴 **실기기 확인이 출시 뒤로 밀렸다** — 특히 «알람이 이틀 연속 오는가»는 아직 안 봤다.
> - **(이전 게시본) v1.6.1 (versionCode 7) — 2026-08-12 출시.** AI 모델 하드코딩 장애가 이 배포로 해소됐다(08-11 업로드 → 하루 만에 통과). **3개 앱 전부 최신 라이브**: KDailyUtil **v1.6.1(vc7)** · KLotto645 **v1.0.3(vc13)** · K장부 **v1.0.1(vc2, 08-11)**.
> - **⏳ 출시 후 남은 확인 3건**: ① 스토어 '데이터 보안' 칸에 *공유되는 데이터: 사진, 기타 사용자 제작 콘텐츠* 반영 ② **`aiModel` 원격 레버 검증**(코드에만 있고 한 번도 안 당겨봄 — K장부가 만든 회귀 테스트 이식이 수동 확인보다 낫다) ③ **깨끗한 계정으로 신규 설치 확인**(이 장애는 기존 기기 테스트로 검증되지 않는다). 상세 = [NEXT_SESSION.md](NEXT_SESSION.md) §할 일 1.
> - **(해소됨) 2026-08-07 긴급 수정 — v1.6.1로 배포 완료**: `GeminiManager`의 `gemini-2.5-flash` 하드코딩 제거. 이 모델은 **신규 사용자에게 404**(*"no longer available to new users"*)라서, **v1.6을 새로 설치해 자기 키를 넣은 사용자는 AI 13종이 전부 실패**한다. 기존 계정은 되기 때문에 개발 중엔 안 보인다. → 별칭 기본값(`gemini-flash-latest`) + 404 전용 폴백 + resolved 기억 + `family.json`의 `aiModel` 원격 지정. 전 기능이 `ask(prompt)` 한 곳을 통과한다. **v1.6.1(08-12 라이브)로 배포 완료.**
...
> - **다음 세션 착수점**: ① ~~v1.6.1 빌드·업로드~~ ✅ **출시 완료(08-12)** ② ~~방침 배포 사본 교체~~ ✅ 완료(08-10) ③ ~~데이터 보안 양식 대조~~ ✅ 완료(08-11, **실제로 어긋나 있어 수정함**) ④ ⏳ **`aiModel` 원격 레버 검증**(이제 가능) ⑤ ~~K장부 데이터 보안 점검~~ ✅ **그 세션이 완료(08-11)** ⑥ (선택) **AI 체험판 이식 — 다음 실제 작업 후보 1순위**. 상세 = [NEXT_SESSION.md](NEXT_SESSION.md) · [AI_KEY_NOTES.md §4](AI_KEY_NOTES.md) · 정책 = [PLAY_POLICY_COMPLIANCE.md](PLAY_POLICY_COMPLIANCE.md).
> - **🛡 Play 정책 준수(3앱 공용) 문서 신설(2026-08-11)** = [PLAY_POLICY_COMPLIANCE.md](PLAY_POLICY_COMPLIANCE.md). **Android 개발자 인증**(2026-09-30 기한 — ✅ 패키지 7개 전부 등록됨, 할 일 없음) · **데이터 보안 양식을 방침과 일치시키는 판단 기준**(AI 전송앱: `공유됨` ✅ / `임시 처리` ❌ / `선택` / 목적 `앱 기능`) · 미사용 앱 4건 방치 결정 · **K장부·KLotto 점검 항목**.
> - **v1.6.1 (versionCode 7) — 🎉 2026-08-12 출시·라이브.** 산출물 = [`app/release/kdailyutil-v1.6.1.aab`](../app/release/kdailyutil-v1.6.1.aab) (11,225,422 bytes, 업로드 키 서명 검증 완료). (이전 v1.6=vc6 08-04 / v1.5=vc5 07-23 / v1.4=vc4 07-21 / v1.2=vc3 07-08 / v1.1=vc2.) **다음 업로드는 vc8** — versionName은 변경 성격에 따라 정한다(위 § 버전 스킴). `versionCode`는 반려에도 소모되며 되돌릴 수 없다.
> - **버전별 내용**: v1.6.1 = AI 모델 하드코딩 장애 수정(위) · v1.6 = 자매앱 동적 레지스트리(§8) 실사용 개시. 출시 노트 문구 = [`app/release/RELEASE_NOTES.md`](../app/release/RELEASE_NOTES.md).
> - **🎉 K-시리즈 배포 현황(2026-08-12)**: 3개 앱 전부 라이브 + **전부 최신판**. 자매앱 동적 레지스트리(§8)가 전 앱에서 실효 — 이후 신규 자매앱 추가에 어떤 앱도 재배포하지 않는다. `comingSoon` 전환은 **정본·번들·라이브 3곳 반영 완료**(08-10, JSON 한 줄로 전 앱 반영되는 것을 실제 확인).
>   - 🔴 **`family.json` 최상위엔 앱별 비상 레버가 얹혀 있다**(`aiModel`=이 앱, `aiTrial`·`fscApi`=K장부). **라이브를 덮어쓰기 전에 라이브 최상위 키를 먼저 확인**할 것 — 확인 없이 정본을 올리면 껐던 기능이 조용히 되살아난다. 레지스트리·절차 = [family_config/README.md](family_config/README.md) **§3-1**.
...
- **최소 SDK**: 26 / 타겟 SDK: 36
- **버전**: 소스 = versionCode **10** / versionName **1.7.2** = **게시본과 같다**(🎉 2026-09-17 출시·라이브 · 09-16 업로드). 🔴 **다음 업로드는 vc11**. 스킴 = 유의적 버전(아래 § 참고, `versionCode`와 분리). ⚠️ **1.7.1·1.7.2는 두 판 연속 예외다** — 기능 추가가 섞여 규칙대로면 MINOR인데 사용자가 PATCH로 결정했다(09-08 · 09-16). 다음 판은 다시 규칙대로 매긴다.
