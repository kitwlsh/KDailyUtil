...
> ### 🚩 지금 상태 (신규 세션 필독)
> - 🆕 **업로드 대기본 = v1.7.0 (versionCode 8)** — 2026-09-07 빌드·서명 확인 완료. 실기기 확인 뒤 업로드.
>   들어간 것 = 매일 새 지문 공급(기능) + 「새 문제 N개」 상한·복귀 사면 + 리텐션 한 판 + 503 대응.
> - **🎉 게시본 = v1.6.1 (versionCode 7) — 2026-08-12 출시·라이브.** AI 모델 하드코딩 장애가 이 배포로 해소됐다(08-11 업로드 → 하루 만에 통과). **3개 앱 전부 최신 라이브**: KDailyUtil **v1.6.1(vc7)** · KLotto645 **v1.0.3(vc13)** · K장부 **v1.0.1(vc2, 08-11)**.
...
- **최소 SDK**: 26 / 타겟 SDK: 36
- **버전**: 소스 = versionCode **8** / versionName **1.7.0** = **업로드 대기본**(2026-09-07 빌드). 게시본은 vc7/1.6.1(08-12 출시). 스킴 = 유의적 버전(아래 § 참고, `versionCode`와 분리). 🔴 1.6.2가 아닌 이유 = 지문 공급이 **기능 추가**여서 MINOR가 올라갔다.
