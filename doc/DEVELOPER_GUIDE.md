...
> - **v1.1 이후 주요 변경**: 오디오 SAF 복구·미디어버튼·인터럽트 재개 / 뉴스 저작권 보수화(본문 비스크랩·낭독 스니펫·쉐도잉 배움터 이전) / 퀴즈 중복방지 3중화·AI 퀴즈 가이드·커스텀 편집·오류신고 게이팅 / 증시 '실적 뉴스·전망' 개편·**과거 실적 조회+회사 검색**·조회기간 AI캐시 복원·DART 키 도움말 / 탭 ＋빠른추가.
...
> 최신 상태는 항상 `git log --oneline -20` 으로 확인. **`8f51481`(v1.1)까지가 스토어 배포본**, 그 위(2026-06-29~07-06)는 미배포(다음 v1.2).
