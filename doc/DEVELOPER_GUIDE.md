...
- **최소 SDK**: 26 / 타겟 SDK: 36
- **버전**: 소스 = versionCode **9** / versionName **1.7.1** = **2026-09-08 업로드·검토 중**. 게시본은 vc8/1.7.0(09-08 출시). 스킴 = 유의적 버전(아래 § 참고, `versionCode`와 분리). ⚠️ **1.7.1은 예외다** — 기능 추가가 섞여 규칙대로면 1.8.0인데 사용자가 PATCH로 결정했다(2026-09-08). 다음 판은 다시 규칙대로 매긴다.
