...
> - **다음 세션 착수점**: ① ~~v1.6.1 빌드·업로드~~ ✅ **완료(08-11) — 🕐 검토 중** ② ~~방침 배포 사본 교체~~ ✅ 완료(08-10) ③ ~~데이터 보안 양식 대조~~ ✅ 완료(08-11, **실제로 어긋나 있어 수정함**) ④ ⏳ **검토 통과 후 `aiModel` 원격 교체 실전 검증** ⑤ **K장부 데이터 보안 점검 넘기기**(그 세션) ⑥ (선택) 체험·킬스위치 이식. 상세 = [AI_KEY_NOTES.md §4](AI_KEY_NOTES.md) · 정책 = [PLAY_POLICY_COMPLIANCE.md](PLAY_POLICY_COMPLIANCE.md).
