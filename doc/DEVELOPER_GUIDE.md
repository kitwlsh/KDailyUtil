...
> ### 🚩 지금 상태 (신규 세션 필독)
> - **스토어 배포 버전 = v1.1 (versionCode 2)**. 이후 커밋(아래 목록)은 **모두 git에 푸시됐지만 아직 스토어 미배포** = **다음 버전(v1.2 예정)** 분입니다.
> - **다음 스토어 업로드 전 반드시 버전 상향**: `versionCode 3 / versionName 1.2` ([app/build.gradle.kts](../app/build.gradle.kts)). *(사용자 요청으로 아직 안 올림)*
> - working tree 깨끗, `origin/main` 동기화됨.
> - **v1.1 이후 주요 변경**: 오디오 SAF 복구·미디어버튼·인터럽트 재개 / 뉴스 저작권 보수화(본문 비스크랩·낭독 스니펫·쉐도잉 배움터 이전) / 퀴즈 중복방지 3중화·AI 퀴즈 가이드·커스텀 편집·오류신고 게이팅 / 증시 '실적 뉴스·전망' 개편·**과거 실적 조회+회사 검색**·조회기간 AI캐시 복원·DART 키 도움말 / 탭 ＋빠른추가 / **퀴즈 데이터 파싱 내구성(불량 1건이 카테고리 전체를 안 깨게)·상식백과 복구·데이터 검증 CI 게이트·퀴즈/독서 뒤로가기 처리(2026-07-07)**.
> - **다음 후보**: 버전 상향+스토어 업로드, 실기기 최종 점검, (선택) 키워드 순서변경(Set→List), 과거실적 추세 종합 AI 코멘트, (선택) startQuiz 정답-빈문항 dedup 제외 보완.
...
- **최소 SDK**: 26 / 타겟 SDK: 36
- **버전**: versionCode 2 / versionName 1.1 (스토어 배포본) — 이후 변경은 미배포(다음 v1.2)
...
- [ ] (선택) **startQuiz 정답-빈문항 dedup 보완**: `QuizViewModel.startQuiz`의 정규화-정답 기준 dedup이 정답이 빈 문항을 조용히 제외함(시각 퀴즈 등 잠재 취약, 현재 미발동). 질문 기준 dedup으로 보완 검토.
...
**2026-07-07 세션 (배움터 퀴즈 안정화·뒤로가기, 모두 푸시)**
| 커밋 | 내용 |
|------|------|
