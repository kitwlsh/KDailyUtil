...
## 3-1. 실측 결과 (2026-08-25, 실제 키 · HTTP 직접 호출) — **503 대응을 넣으면서 재측정**

> 🔑 **왜 다시 쟀나**: 폴백 후보를 `gemini-3.6-flash`로 바꾸면서 그 근거가 **K장부의 08-18 실측**이었다.
> 이번 사고 자체가 *「목록에 없는 모델이 폴백에 박혀 있던」* 문제였으므로, 남의 측정값을 그대로 믿고
> 넣으면 같은 실수를 반복한다. **폴백 후보에 넣기 전에는 반드시 직접 확인한다.**

| 모델 | 결과 | 08-07 대비 |
|---|---|---|
| **`gemini-flash-latest`** (기본) | ⚠️ **200과 503이 섞인다** (아래 참조) | 그때는 200만 나왔다 |
| `gemini-3.5-flash` (폴백 1) | ✅ 200 · 1.7~2.9초 | 동일 |
| `gemini-3.6-flash` (폴백 2) | ✅ 200 · 3.4초 | **신규 확인** |
| `gemini-3.7-flash` | ✅ 200 · 3.4초 | 목록에 새로 생겼다(폴백 미포함) |
| `gemini-2.0-flash` | ❌ **404 `no longer available`** | **429였다가 아예 사라졌다** |
| `gemini-2.5-flash` | ❌ 404 `no longer available to new users` | 동일 |

`models` 목록(`generateContent` 지원) 37종 중 flash 계열 18종. **`gemini-2.0-flash`는 목록에 없다** — 폴백에서 뺀 판단이 맞았다.

### 🔴 가장 중요한 결과 — 503은 «상시 장애»가 아니라 **간헐적**이다

`gemini-flash-latest`를 21회 호출한 결과 **200이 대부분이고 503이 4회 섞여 나왔다**(약 19%).
같은 모델이 1.35초 만에 503을 주기도 하고 1.7초 만에 200을 주기도 한다.

**이것이 이 장애가 넉 달 가까이 안 잡힌 이유다.** 상시 장애라면 즉시 발견됐겠지만,
- 개발자가 시험할 때는 대개 통과한다 → *「잘 되는데?」*
- 사용자는 **5번에 1번꼴로** 영문 오류를 본다 → *「가끔 안 돼요」* (재현이 안 되는 신고)

> ⚠️ **수정 전(vc7)에는 그 19%가 전부 하드 실패였다.** 지금은 폴백이 받아 `gemini-3.5-flash`로 넘어간다.
> 사용자 입장에서 달라지는 것 = **1.7~2.9초 더 기다리고 정상 응답을 받는다.**

### 부수 확인 — 무료 등급 분당 한도는 생각보다 낮다
빠르게 연속 호출하자 **9회쯤에서 429가 걸렸다**(그 뒤로는 전부 429). 두 가지 함의가 있다:
- **§4-3 체험판 설계에 직접 영향** — «체험 N회»를 총량으로만 잡으면 사용자가 **한도에 먼저 걸린다**
- **429에 자동 재시도를 붙일 때 주의** — 겹쳐 걸면 한도만 더 태운다(그래서 이번에 안 넣었다. `StockViewModel`이 이미 자기 백오프를 갖고 있다)

**측정 방법**(K장부 `doc/OPERATIONS.md` §3-1-2와 동일):
```bash
KEY=$(grep '^gemini.default.key=' <KJangbu>/local.properties | cut -d= -f2-)
curl -s "https://generativelanguage.googleapis.com/v1beta/models?key=$KEY"      # 실재 확인
curl -s -o /dev/null -w "%{http_code} %{time_total}\n" -H "Content-Type: application/json" \
  -d '{"contents":[{"parts":[{"text":"OK"}]}]}' \
  "https://generativelanguage.googleapis.com/v1beta/models/<모델>:generateContent?key=$KEY"
```
⚠️ 이 키는 **K장부의 개발자 키**(기존 계정)다 → **신규 계정 경로는 이것으로 검증되지 않는다**(§4-1 확인 3번은 여전히 남아 있다).

## 3-2. 🔴 별칭이 «한도가 짠 모델»로 옮겨갔다 (2026-09-08 · 실제 장애로 드러남)

### 무슨 일이 있었나

사용자가 퀴즈·지문 로봇을 **수동으로 3번 실행했고 3번 다 실패**했다. 로봇이 남긴
`korean_quiz_data/last_run.json`(퀴즈·지문 **양쪽 블록 모두** `"status": "failed"`)에 사유가 그대로 있었다:

```
429 You exceeded your current quota
Quota exceeded for metric: generativelanguage.googleapis.com/generate_content_free_tier_requests,
limit: 20, model: gemini-3.8-flash
Please retry in 8.405707679s
```

### 🔴 여기서 드러난 것 — 별칭은 «어느 모델로 풀릴지» 우리가 못 정한다

로봇도 앱도 기본값이 **별칭 `gemini-flash-latest`** 인데, 이 오류 메시지가 **그 별칭이 지금
`gemini-3.8-flash`로 풀린다**는 것과 **그 모델의 무료 등급 한도가 20**이라는 것을 함께 알려줬다.

- §3-1(08-25)에 실측한 것은 `gemini-3.5-flash`·`gemini-3.6-flash`였고 둘 다 멀쩡했다
- **«모델 이름을 버전으로 박지 말 것»(08-12 교훈)은 여전히 맞다.** 별칭이 없었다면 404로 죽었을 것이다
- 🔴 **다만 별칭은 «한도가 훨씬 짠 신모델»로도 옮겨간다.** 이 경우는 대비가 없었다 —
  기존 방어(404 폴백·503 폴백)는 **429를 잡지 않기 때문이다**(아래)

### 로봇이 태운 것이 아니다 (호출 수 확인)

- `update_quiz.py`는 `generate_content`를 **1회**만 부른다(L252, 4개 카테고리를 한 번에 만든다)
- `update_passages.py`도 **1회**
- 429면 폴백 후보를 더 태우지 않고 **즉시 중단**한다 → 실패한 실행 1회 = **호출 1회**

→ 3회 실행 = **최대 6회**. 한도 20을 채운 것은 **다른 사용**이다. 같은 키를 앱이 쓰고 있었다면
그날 기기 확인에 쓴 AI 호출이 유력하다.

### ⏰ 한도가 풀리는 시각 — 태평양 자정 = **한국 오후 4시**

무료 등급 일일 한도는 **태평양 시간 자정**에 리셋된다. 한국은 16시간 빠르다(서머타임 PDT 기준).

| 기간 | 태평양 자정 = 한국시간 |
|---|---|
| 서머타임(3월 둘째 일요일 ~ 11월 첫째 일요일) | **오후 4시** |
| 겨울(11월 첫째 일요일 ~) | **오후 5시** |

→ 09-08에 막혀 있던 «하루»는 **09-07 16:00 KST ~ 09-08 16:00 KST** 구간이었다.

### 🔴 미해결 — 429에 폴백해야 하는가 (**실측 뒤에 결정할 것**)

[GeminiManager.kt](../app/src/main/java/com/kitwlshcom/kdailyutil/data/remote/GeminiManager.kt)는
404·503에는 다음 모델로 넘어가지만 **429에는 일부러 넘어가지 않는다.** 주석의 근거는 이것이다:

> 「429(내 한도)는 폴백하지 않는다 — **한도는 키 단위라 모델을 바꿔도 그대로다**」

**그런데 이번 오류가 그 전제를 반박한다** — `limit: 20, model: gemini-3.8-flash`처럼
**한도가 모델 이름과 함께** 나온다. 모델별로 별도 한도라면 429에도 폴백이 유효하다.

🔴 **켜기 전에 반드시 실측할 것.** 전제가 틀려 «한도는 역시 키 단위»였다면,
폴백을 켜는 순간 **한도가 소진된 상태에서 모델만 계속 바꿔 태우는** 반대 사고가 된다.

**실측 절차** (한도가 이미 소진된 상태에서 해야 의미가 있다 — 즉 **오후 4시 리셋 «전»**):

```bash
KEY=<Gemini 키>
# A. 한도에 걸린 모델이 정말 429를 주는지
curl -s -H "Content-Type: application/json" -d '{"contents":[{"parts":[{"text":"OK"}]}]}' \
  "https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=$KEY"
# B. 같은 키로 «다른 모델»은 되는가  ← 이 한 줄이 판단의 전부다
curl -s -H "Content-Type: application/json" -d '{"contents":[{"parts":[{"text":"OK"}]}]}' \
  "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=$KEY"
```

- **B가 200이면** → 한도는 **모델별**이다 → 429 폴백을 넣는다(vc10)
- **B도 429면** → 한도는 **키 단위**다 → 지금 동작(폴백 안 함)이 맞다. 주석을 그대로 두고 이 절만 갱신한다

### 🟡 같이 고칠 것 — 429 안내 문구가 사실과 다르다

지금 문구(`aiErrorMessage`)는 이렇다:

> *"이 키의 사용 한도를 초과했습니다. 무료 등급은 분당·하루 한도가 있어 **잠시 뒤 풀립니다**."*

**«잠시»가 아니다.** 분당 한도면 몇 초지만 **일일 한도면 최대 24시간**이다.
사용자는 몇 분 기다렸다 다시 눌러 보고, 또 안 되니 **「앱이 고장났다」**고 판단한다.
이 문구 수정은 실측 결과와 **무관하게** 맞는 수정이다(vc10).

### 🔧 로봇 쪽 즉시 대응 — 코드 수정 없이 되는 것

`korean_quiz_data` → Settings → Secrets and variables → Actions → **Variables** 탭 →
**`QUIZ_MODEL`** 에 모델명(예: `gemini-3.5-flash`) → Actions에서 수동 실행.
**퀴즈·지문 스크립트가 같은 레버를 읽는다.** 확인이 끝나면 **값을 지워 원상복구**한다
