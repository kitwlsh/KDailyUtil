...
### 🎙 오디오 캡처 및 재생 시스템 (New)
*   **3단계 스마트 레코딩**: Idle(정지) - Ready(준비/녹색) - Recording(녹음/빨간색)의 3단계 상태를 통해 백그라운드 버튼 노출을 완벽 제어.
*   **백그라운드 연속 녹음**: 다른 앱 사용 중에도 플로팅 버튼을 통해 여러 번의 녹음-정지 반복 가능.
*   **끊김 없는 백그라운드 재생**: 포그라운드 서비스 및 WakeLock 기술을 적용하여 화면이 꺼져도 안정적인 오디오 재생 보장.
*   **전문가급 UI/UX**:
    *   하단 탭바 초밀착 미니 플레이어 (이전/다음 곡, 재생 모드, 편집 잠금 제어).
    *   현재 재생 곡 위치로 리스트 자동 스크롤 (Auto-Scroll).
    *   데이터 유실 방지를 위한 편집 잠금(Edit Lock) 기능 및 설정 저장(DataStore).
