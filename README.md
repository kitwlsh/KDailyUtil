...
| 항목 | 값 |
|---|---|
| **스토어 게시본** | **v1.7.0 (versionCode 8)** — 🎉 **2026-09-08 출시·라이브**(09-07 업로드 → 다음날) |
| 🕐 **업로드 완료 · 검토 중** | **v1.7.1 (versionCode 9)** — 2026-09-08 업로드. [`app/release/kdailyutil-v1.7.1.aab`](app/release/kdailyutil-v1.7.1.aab) (11,348,510 bytes) · 서명 SHA-256 `61:12:DE:…:A5:12:99` 확인 완료 · ✅ 실기기 확인 완료. 🔴 **vc9 소모됨 → 다음 업로드는 vc10** |
| 빌드 상태 | 단위 테스트 **82건 통과** · `:app:bundleRelease`·`:app:compileReleaseKotlin` 성공 |
| 남은 일 | 심사 통과 확인 · 🔴 vc8 몫이던 실기기 확인(**알람 이틀 연속** · `aiModel` 레버 · 데이터 보안 칸 · 깨끗한 계정 신규 설치) · 🔴 **vc10 후보 = AI 429 대응** — [doc/NEXT_SESSION.md](doc/NEXT_SESSION.md) |

**v1.7.1(vc9)에 들어 있는 것 — 독서 훈련 「시작까지 가는 길」을 줄였다(2026-09-08)**
1. 📌 **지문을 고른 그 자리에서 훈련이 시작된다** — 예전 「이 지문으로 훈련하기」는
   **이름과 달리 훈련을 시작하지 않고 지문만 바꿔서**, 누른 뒤 훈련 목록까지 화면 두세 장을 내려가야 했다
2. 🔴 **버튼 이름이 무엇이 열리는지 말한다** — 「▶ 리듬 페이서 시작」 + 「▾」로 다른 훈련 선택
3. 🔁 **결과 화면에 「다시」·「다음: <훈련명>」** — 허브를 거치지 않는다
4. 🔴 **훈련을 마치고 돌아오면 스크롤이 그 자리다** — 예전에는 맨 위로 튀었다
5. 📚 **보관함 20편 + 「더 보기」** — 상한이 없어 지문을 넣을수록 훈련 버튼이 멀어지던 구조를 끊었다

> ⚠️ **버전명이 `1.8.0`이 아니라 `1.7.1`인 이유**: 스킴 규칙(`기능추가 → MINOR`)대로면
> 이번 판은 1.8.0이다(시작 버튼·마지막 훈련 기억·결과 화면 버튼이 기능 추가다).
...
| **AI 키 현황·남은 일** | [AI_KEY_NOTES.md](doc/AI_KEY_NOTES.md) | 🔴 **모델 하드코딩 장애**(신규 사용자 AI 전면 실패)와 4중 방어·키 안내 개선 + **모델 실측표 2종**(§3 08-07 / **§3-1 08-25 — 503은 상시가 아니라 간헐적(21회 중 4회)이라는 실측, 무료 등급 분당 한도 실측**) + 🔴 **§3-2(09-08) — 별칭이 «한도가 짠 모델»로 옜겨가 로봇이 3연속 실패한 건**(429 일일 한도 · 한도 리셋 = 한국시간 오후 4시 · **429 폴백을 켜야 하는가의 실측 절차**) + **남은 일**(✅v1.6.1 출시완료 / ⏳`aiModel` 레버 검증 · 체험/킬스위치 이식 시 주의). 정책 단일 기준은 `KJangbu/doc/AI_KEY_POLICY.md` |
...
- 📖 **오늘의 지문**(v1.7.0): 로봇이 매일 1편을 공급하고, 앱이 날짜로 정해진 1편을 「오늘의 지문」으로 준다. 「지문 고르기」로 서버 지문을 골라 쓸 수도 있다 → [FEATURE_DAILY_PASSAGES.md](doc/FEATURE_DAILY_PASSAGES.md)
...
> 최신은 위쪽에 있다. 게시본은 **v1.7.0(vc8, 2026-09-08 라이브)** 이고,
> **2026-09-08의 독서 훈련 동선 작업은 v1.7.1(vc9)로 같은 날 올려 검토 중이다.**
> 🟢 **저장소에 밀린 코드는 없다.**

### 📌 독서 훈련 「시작까지 가는 길」을 줄였다 (2026-09-08 · 🕐 v1.7.1 업로드·검토 중)

원인 분석·설계 판단 = [doc/NEXT_SESSION.md §할 일 0-B](doc/NEXT_SESSION.md) · 확인 절차 = 같은 문서 부록 A §E

사용자 신고에서 출발했다 — *「내 지문이 많으면 스크롤도 많이 올려야 하더라구요.
훈련 하나 끝나고 다음 훈련 하려면 또 스크롤 그만큼 올려야 하구요.」*
코드를 읽어 원인 3가지를 확인했다: ① 훈련 버튼이 허브 맨 끝(보관함이 비어도 화면 1.5장, 20편이면 3.3장)
② 보관함에 표시 상한이 없어 지문을 넣을수록 훈련이 멀어짐 ③ 🔴 **허브로 돌아오면 스크롤이 맨 위로 리셋**
(`rememberScrollState()`가 `ReadingHub` 안에 있어 `when(module)`이 허브를 들어낼 때 위치가 버려졌다).

- 📌 지문 다섯 곳(오늘의 지문·새로 온 지문·지문 고르기·연습 지문·보관함) 전부에 **시작 버튼**.
  예전 「이 지문으로 훈련하기」는 **이름과 달리 훈련을 시작하지 않고 지문만 바꿨다**
- 🔴 **버튼 이름이 무엇이 열리는지 말한다** — 「▶ 리듬 페이서 시작」 + 「▾」로 다른 훈련 선택.
  마지막에 한 훈련을 기억하고, 처음 쓰는 사람에게는 기본값으로 몰래 시작하지 않는다
- 🔁 결과 화면에 **「🔁 <훈련명> 다시」·「▶ 다음: <훈련명>」** — 허브를 거치지 않는다
- 🔴 스크롤 위치 보존 + 배움터 서브탭 기억 · 📚 보관함 20편 + 「더 보기」
- 🔴 **일부러 안 한 것**: 허브 순서 재배열 · 하단 고정 시작 바 · 시작 확인 대화창 · 모든 줄에 시작 버튼
  (판정 근거는 위 문서에 있다 — 다음 세션이 「친절하게」 되돌리지 않도록)
- 검증: 단위 테스트 **9건 신설 → 82건 통과** · 🔬 뮤테이션 2건이 정확히 실패 · ✅ 실기기 확인 완료(사용자)

### 📖 매일 새 지문이 도착한다 — 독서 훈련 지문 공급 (2026-09-07 · ✅ v1.7.0으로 출시됨)

설계·구현 결과 = [doc/FEATURE_DAILY_PASSAGES.md](doc/FEATURE_DAILY_PASSAGES.md)

- 🔴 **지문이 앱에 하드코딩된 19편뿐이었다.** 매일 하면 19일에 한 바퀴가 돌고 그 뒤로는 재독인데,
  재독이면 WPM이 오른다 — 그건 읽기 실력이 아니라 **내용을 이미 아는 것**이라 앱이 보여 주는
  **WPM 추이 그래프가 «실력»이 아니라 «몇 번째 재독인가»를 그리게** 된다. 「랜덤 지문」에는 «오늘 것»도 없었다.
- **공급**: 퀴즈 로봇이 있는 저장소([korean_quiz_data](https://github.com/kitwlsh/korean_quiz_data))에 `update_passages.py`를 새로 뒀다 —
  하루 1편 · 150~250자 · 17주제 로테이션 · `passages_<연도>.json`(연도 분할) · 저작권·중복 가드.
  🔴 **퀴즈와 별도 스크립트·별도 스텝**이다(한쪽 실패가 다른 쪽을 끌고 죽지 않게). 생존 신호에도 `passages` 블록이 따로 붙는다.
- **수신**: 「오늘의 지문」 = **날짜로 정해지는 1편**(퀴즈가 쓰는 `pickDailyIndices` 재사용, 새것이 있으면 그쪽에서 뽑는다) ·
  「지문 고르기」(서버 지문 전체, 20편씩) · 「랜덤 지문」이 서버 지문까지 포함 · 보관함으로 «복사» · 「치우기」=숨김 ·
  **「오늘 완료」 표시** · 알림에 「새 지문 N편」 파트(우선순위 지문→문제→연속일, **최대 2파트**)
- 🔴 **로봇 지문과 사용자 보관함은 다른 파일이다** — 섞으면 동기화 버그 한 번에 사용자가 촬영해 넣은 지문이 날아간다.
- 🟢 **내장 19편은 지우지 않았다** — 오프라인·첫 설치에서 떨어질 바닥이다.
- **앱을 다시 내지 않아도 지문이 계속 늘어난다.** 이번 판에 «받는 쪽»이 들어갔기 때문이다.
- 검증: 단위 테스트 **73건 통과**(신설 21건) · 뮤테이션 시험 3건 · 로봇은 스텁으로 **7개 시나리오 실제 실행**.

### 🔴 「새 문제 N개」에 상한과 복귀 사면 (2026-09-07 · ✅ v1.7.0으로 출시됨)

- 상한이 없어 **두 달 비운 사용자에게 「새 문제 300개」**가 나갔다. 하루 5문항이 들어오므로
  **오래 비운 사람일수록 큰 숫자를 받는** 구조 — 돌아오기 가장 어려운 사람에게 가장 큰 벽을 세우고 있었다.
- 표시 상한 20(`「20개+」`) + **7일 이상 비웠으면 숫자를 아예 말하지 않고** 「그동안 새 문제가 쌓였어요 — 오늘 한 판부터 다시 시작해요」.
  앱을 열면 기준점을 리셋해 「밀린 것」 개념 자체를 지운다. 「놓친 N개」·「끊겼습니다」는 쓰지 않는다(죄책감 알림 = 알림 해제 = 영구 이탈).

### 🔁 매일 한 번은 들어오게 — 리텐션 한 판 (2026-09-04 · ✅ v1.7.0으로 출시됨)
...
### 🔥 503(AI 서버 과부하) 대응 (2026-08-25 · ✅ v1.7.0으로 출시됨)
