...
> ⏳ **다음 = v1.6.1(vc7) — 🔨 AAB 빌드·서명 확인 완료(2026-08-11). Play Console 업로드만 남음.** 문구는 아래 'v1.6.1' 절.
> 🔄 **버전 스킴 변경(2026-08-11)**: 옛 규칙 "versionName 끝자리 = versionCode"를 **폐기**하고 **유의적 버전(MAJOR.MINOR.PATCH)** 으로 전환했다.
> `versionCode`(Play용 정수·업로드마다 증가)와 `versionName`(표시용)을 **묶지 않는다.** 버그수정→PATCH / 기능추가→MINOR / 호환깨짐→MAJOR.
> 그래서 이번 장애 수정판은 **`1.7`이 아니라 `1.6.1`** 이다(vc는 예정대로 7). 규칙·전환 이유 = [DEVELOPER_GUIDE.md § 버전 스킴](../../doc/DEVELOPER_GUIDE.md).
...
## 📌 Google Play '출시 노트' 붙여넣기용 문구 — v1.6.1 (⏳ 업로드 대기)
...
>
> 📛 **버전명 주의**: 이 릴리스는 08-10에 `1.7`로 빌드했다가, **순수 버그 수정이라 `1.6.1`이 맞다**는 판단으로
> 08-11에 **버전 스킴을 유의적 버전으로 전환하고 재빌드**했다(`versionCode`는 예정대로 **7** 그대로).
> 스토어에 `1.7`이 올라간 적은 **없다.** 옛 문서·커밋에 남은 "v1.7"은 모두 이 `1.6.1`을 가리킨다.
>
> **빌드 완료(2026-08-11)** — `kdailyutil-v1.6.1.aab` (11,225,422 bytes)
> · versionCode 7 / versionName **1.6.1** (병합 매니페스트에서 확인)
...
• AI에 보낸 내용이 어떻게 처리되는지에 대한 안내를 추가했어요.
...
> 스토어 게시본 v1.2(vc3, 07-08) 이후의 변경을 모아 배포하는 버전. (versionName 끝자리를 versionCode(4)와 맞추며 1.3은 건너뜀 — **이 스킴은 2026-08-11에 폐기됐다.** 당시 기록)
