...
3. 결과를 앱에 넣습니다. (둘 중 편한 방법)
   - 파일이 되는 AI(예: ChatGPT 코드 실행): .kquiz 파일을 내려받아
     앱 → 배움터 → 우리말 퀴즈 → '📥 외부 퀴즈 패키지(.kquiz) 가져오기'.
   - 텍스트(JSON)만 주는 AI(예: 대부분의 무료 챗봇): 출력된 JSON을 '전체 복사'한 뒤
     앱의 '📋 텍스트(JSON) 붙여넣기로 가져오기'에 붙여넣으면 끝. (파일 저장 불필요)

* 참고: 파일로 되냐 텍스트로만 나오냐는 그 AI의 기능 차이입니다. 텍스트만 나와도
  위 '붙여넣기로 가져오기'를 쓰면 파일을 따로 저장할 필요가 없습니다.
